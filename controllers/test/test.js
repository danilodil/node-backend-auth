...
var g_gtmAccount, g_productCode, g_quoteState, g_sessionId, g_quoteKey, g_quoteNumber, g_agentCode, g_retrieval, g_quoteSource, g_domain = getDomainName(),
    g_restrictedFieldList, g_polSoldInd, g_appID, g_pageTitle, g_aqarFarPage, g_wasRated, g_vehCount, g_drvrCount, g_uccType;
dataLayer = [{
...
function getDomainName() {
    ...
    if (document.domain != null) {
        ...
        if (parts.length > 1) domainName = "." + (parts[parts.length - 2] + "." + parts[parts.length - 1])
    }
    return domainName
}
var gaObject = {};
...
var g_changes = [],
    g_childSelectLists = ["veh_make", "veh_mdl_nam", "veh_sym_sel"],
    g_disabledClass = "disabled",
    g_regExpValid = /^[a-zA-Z\d\!\@\#\$\%\&\*\(\)\_\+\-\=\\\:\"\;\'\<\>\?\,\.\/ \n\r]*$/,
    ...
    g_dependantFields = 0,
    g_controlPrefixes = [""],
    g_masterPreFunc = null,
    g_masterPostFunc = null,
    g_masterindex = null,
    g_prefixSeparator = "~",
    g_updatechildFlag = "",
    pageFieldXML = null,
    ...
    OnPageErrorsButtonId, onpgEdts, DisplayContinueButtonId, g_closeWindow = true,
    historyChanged = false,
    BACKSPACE_KEY = 8,
    DELETE_KEY = 46,
    NUMBERPAD_DELETE_KEY = 127;
...
function UpdateDependencies(changedObj) {
    ...
    if (IsDataIslandAttribute != null && IsDataIslandAttribute != undefined && IsDataIslandAttribute == "coveragesExtractFilter") FilterDataIsland(changedObj, IsDataIslandAttribute, masterXpath);
    else {
        ...
        if (ShouldAddDependentFields(masterNodes[m], changedObj)) {
            ...
            if (dependentNodes != null)
                for (var d1 = 0, d = 0; d < dependentNodes.length; ++d)
                    if (changedObj.CustomFilter == "True" && GetObjValue("prod_cd") == "CA") AddDependentFieldsCA(dependentFields, dependentNodes[d], changedObj.id);
                    else AddDependentFields(dependentFields, dependentNodes[d], changedObj.id)
...
function VisitDependent(filter, masterField, master) {
    ...
    if (master == null) var xPath = "//filters/filter/options[@name='" + GetFieldName(masterField, ".") + "']";
    else {
    ...
    if (options.length > 0)
        for (var i = 0; i < options.length; i++) {
            ...
            if (masterNodes != null && masterNodes.length > 0) var master = masterNodes[0].getAttribute("master"),
...
function CopyOptions(masterField, destField) {
    ...
    if (_bIsRadio) return;
        ...
        for (var newOption = null, i = 0; i < kids.length; i++) {
            ...
            if (opt.text != null) newOption.text = opt.text;
            else newOption.text = opt.getAttribute("value");
...
function AddDependentFields(dependentFields, dependentNode, masterFieldId) {
    ...
    for (var parts = dependentFieldId.split("/"), fieldName = parts[0], p = 1; p < parts.length; p++) {
        var index = "*";
        if (entities != "*" && p <= masterIndices.length) index = masterIndices[p - 1];
...
function ProcessQueue() {
    ...
    } else g_transactionInProcess = false
}
...
function ExpandFields(groupNames, startPos, fields, parent) {
    var groupCount = "";
    if (startPos == 0) groupCount = groupNames[startPos] + ".count";
    else groupCount = parent + groupNames[startPos] + ".count";
    ...
    for (var i = 0; i < fieldList.length; i++)
        for (var prefixes = GetControlPrefixes(), fieldId = fieldList[i], j = 0; j < prefixes.length; j++) {
            ...
            if (currObj) {
                ...
                if (currObj.options != null) currObj.options.length > 0 && currObj.options.remove(currObj.options.length - 1)
            }
        }
}
...
function OnFieldHelpComplete(result) {
    try {
        if (result == "SessionExpired") window.location = "sessionexpired.htm";
        else {
...
function xmlToString(xmlData) {
    ...
    if (xmlString == undefined) xmlString = (new XMLSerializer).serializeToString(xmlData);
    return xmlString
}
    ...
    try {
        if (result == "SessionExpired") window.location = "SessionExpired.htm";
            ...
            try {
                ...
                if (exc.message.indexOf("ActiveX") > -1) rsp = (new DOMParser).parseFromString(result, "text/xml");
                else throw exc
            }
        ...
        if (g_masterPostFunc != null)
            if (eval("typeof(" + g_masterPostFunc + ")") == "function") eval(g_masterPostFunc + "('" + g_masterindex + "');");
...
function GetUniqueId(prefix, fieldId) {
    ...
    if (controlId != "") controlId += g_prefixSeparator;
    controlId += fieldId;
    return controlId
}
...
function GetControlSpecificObj(id) {
    var obj = GetObj(id);
    if (obj == null) return;
    if (IsCheckBoxControl(id)) obj = GetObj(id + "_displayCheckBox");
    return obj
}

//JONs METHODS
// const obj = GetObj(id);
// SetFieldValue(obj, value, externalValue) // External value is text
...
function FillDependantObj(id, fieldNode) {
    ...
    if (relNode != null) switch (GetNodeTextFromNode(relNode)) {
        case "R":
        ...
        case "U":
            ...
            } else treatUnknownAs = GetAttribute(obj, "TreatUnknownRelevanceAs");
            if (treatUnknownAs == "Irrelevant") ToggleFieldRelevance(obj, false);
            else ToggleFieldRelevance(obj, true)
    }
    ...
    if (obj.tagName == "SELECT") {
        ...
        if (optionGroup == "true" && getIEVersionNumber() == 0) FillTabletOptions(obj, optionNodes, valueNode);
        else FillOptions(obj, optionNodes, valueNode)
    }
...
function OnFieldInfoError(result) {
    CleanUpFieldInfoTransaction();
    if (result.get_timedOut()) alert("FieldInfoService Timeout: " + result.get_message());
    else {
...
function ToggleFieldRelevance(field, relevant) {
    ...
    if (isHide) {
        ...
        if (parentDiv == null) parentDiv = $get(field.id + "_tr");
        if (parentDiv != null) {
    ...
    if (field.attributes["RuleDisable"]) {
        ...
        var radioColl;
        if (isRadioButton) radioColl = GetRadioCollection(field);
        if (relevant) {
        ...
        if (IsButton(field)) EnableButton(field, relevant);
        else EnableIfDisplayButtonDialog(field, relevant)
    }
}
    ...
    if (field.parentNode != null && field.parentNode.id == "_dbdWrapper")
        if (relevant) {
            ...
            if (button) {
                ...
                if (button.attributes["enabledimage"]) button.src = button.attributes["enabledimage"].value;
            ...
            if (button) {
                ...
                if (button.attributes["disabledimage"]) button.src = button.attributes["disabledimage"].value;
...
function EnableButton(field, flag) {
    if (flag) field.src = field.attributes["enabledimage"].value;
    else field.src = field.attributes["disabledimage"].value
}
...
function EnableCheckbox(field, flag) {
    ...
    if (flag) Sys.UI.DomElement.addCssClass(control, g_disabledClass);
    else {
...
function GetBaseFieldId(obj) {
    if (obj != null && obj != undefined) {
        for (var id = GetFieldId(obj), parts = id.split("."), expr = new String, p = 0; p < parts.length - 1; p += 2) expr += parts[p] + "/";
...
function ClearOptions(obj) {
    if (obj != null) {
        ...
        while (obj.options.length > 0) coll.remove(obj.options.length - 1);
        for (var optGroups = obj.getElementsByTagName("optgroup"), i = optGroups.length - 1; i >= 0; i--) obj.removeChild(optGroups[i])
    }
}
...
function FillOptions(textBox, optionNodes, valueNode) {
    ...
    for (var i = 0; i < optionNodes.length; ++i) {
        ...
        obj.options.add(e);
        if (GetNodeTextFromNode(valueNode) == value) valuenodefound = true
    }
    if (g_updatechildFlag == "d")
    ...
    if (obj.value != "") {
        ...
        if (masterNodes != null && masterNodes.length > 0) {
            var dataIslandFilter = masterNodes[0].getAttribute("DataIslandFilter"),
                ...
                dependentnode = masterNodes[0].getAttribute("dependent");
            FillDependentOptions(GetObj(dependentFieldId), filterDataIsland, currentXPath, master, dependentnode)
        } else FillDependentOptions(GetObj(dependentFieldId), filterDataIsland, currentXPath)
    }
}
...
function GetSelectListIndex(selCtrl, lookUpText) {
    if (selCtrl.options.length <= 0) return -1;
    for (var i = 0; i < selCtrl.options.length; i++)
        if (selCtrl.options[i].text == lookUpText) return i;
    return -1
}
...
function FldOnChange(obj, updateDependencies) {
    if (!InvalidCharacter(obj)) {
        ...
        if (fieldChIdx !== -1) g_changes[fieldChIdx + 1] = obj.value;
        else {
...
function InvalidCharacter(currentField) {
    if (currentField.type == "text" || currentField.type == "textarea")
        if (!g_regExpValid.test(currentField.value)) {
            ...
            if (semicolonPos != -1) fieldCaption = fieldCaption.substr(0, semicolonPos);
            if (fieldCaption == currentField.value || fieldCaption == "") alert("Invalid character in field: " + currentField.value);
            else alert("Invalid character in the " + fieldCaption + " field: " + currentField.value);
            return true
        } return false
}
...
function GetFieldCaption(obj) {
    if (obj != null) {
        ...
        if (fieldName != null && fieldName != "") {
            ...
            if (parsedNameObj != null && parsedNameObj.length > 0) labelNodeID = parsedNameObj[parsedNameObj.length - 1] + "_Label";
            if (labelNodeID != "") {
                var labelNode = GetObj(labelNodeID);
                if (labelNode != null) return labelNode.innerText
            }
        }
    }
    return ""
...
function DebugRequest(changes, dependentFields) {
    if (!GetObj("rqDebug")) return;
    for (var s = new String, d = 0; d < dependentFields.length; ++d) {
        s += dependentFields[d];
        s += "!"
    }
    s += "!";
    for (var c = 0; c < changes.length; ++c) s += changes[c];
    Debug(s, "rqDebug")
}

function Debug(msg, objId) {
    var obj = GetObj(objId);
    if (obj != null) obj.innerText = msg
}
...
function BuildJSONArray(jsArray) {
    ...
    if (jsArray.length > 0) {
        ...
        for (var i = 1; i < jsArray.length; i++)
            if (jsArray[i] != null) result = result + ',"' + jsArray[i] + '"'
    }
    result = result + "]";
    return result
}
...
function OnCleanupComplete(result) {
    if (result == "SessionExpired") window.location = "SessionExpired.htm";
        ...
        window.opener = null;
        if (g_closeWindow) window.close();
        else g_closeWindow = true
    }
}
...
function ShouldAddDependentFields(masterNode, changedObj) {
    ...
    if (cond != null) {
        ...
        if (eval("typeof(" + condFunc + ")") == "function") eval("rv = " + condFunc + "(changedObj)")
    }
    return rv
}
...
function GetAncestor(node, ancestorNodeName) {
    ...
    while (ancestor != null) {
        if (ancestor.nodeName == ancestorNodeName) break;
...
function IsSelectListVehMake(changedObj) {
    ...
    if (make != null && make.tagName == "SELECT") return true;
    return false
}
...
var g_childrenToValidate = ["veh_make"],
    g_childOptionNodes, g_childObject, g_valueNode, g_validationField = "";
...
function CallValidationFunctionIfExists() {
    ...
    if (eval("typeof(" + functionName + ")") == "function") eval(functionName + "()")
}

function ValidationRule_veh_make() {
    var objID = g_childObject.id,
        nameArray = g_childObject.name.split("."),
        yearField, prodField;
...
function GetFieldDisplayedValue(obj) {
    ...
    if (obj) switch (obj.tagName) {
        case "P":
            ...
            break;
        case "SELECT":
            if (obj.selectedIndex != -1) value = obj.options[obj.selectedIndex].text;
            break;
        case "IMG":
            value = obj.src;
            break;
    ...
    switch (obj.tagName) {
        ...
        case "P":
            ...
            if (obj.attributes["BoldWhenValueIs"]) {
                ...
                if (obj.value == boldValue) Sys.UI.DomElement.addCssClass(obj, "bold");
                else Sys.UI.DomElement.removeCssClass(obj, "bold")
            }
            break;
        case "SELECT":
            var listBox = obj;
            if (listBox.length > 0)
                if (listBox.length == 1 && !IsEmpty(listBox[0].text) || listBox.length > 1) listBox.value = value;
            break;
        default:
            var valueText = value;
            if (obj.attributes["mask"]) {
                var temp = GetEmptyMask(obj);
                if (temp != "" && valueText == "") valueText = temp
            }
            obj.value = valueText;
            if (IsRadioButton(obj)) {
...
function IsArray(obj) {
    if (obj != null && typeof obj.tagName == "undefined") return true;
    return false
}
...
function IdentifyPositionedAncestor(obj) {
    ...
    if (parent == document.documentElement || parent.style.position == "relative" || parent.style.position == "absolute") return parent;
    else return IdentifyPositionedAncestor(parent)
}
...
function IsEmpty(StrToCheck) {
    ...
    if (StrToCheck.length == 0) return true;
    else return false
}

function ApplyDateMask(obj) {
    if (!obj.attributes["mask"] || obj.value == "" || obj.value == "__/__/____") return;
    if (obj.getAttribute("mask") == "99/99/9999")
        if (obj.value.substr(0, 10) == "11/11/1111" || obj.value.substr(0, 9) == "11/111111" || obj.value.substr(0, 8) == "11111111") obj.value = "11/11/1911";
        else obj.value = FormatDate("02.02.04", obj.getAttribute("mask"), obj.value, 4)
}
    ...
    for (var c = 0; c < clumps.length; ++c) {
        ...
        if (size > fldWidth[c]) {
            if (clumps.length < expectedClumps) {
                for (var split = clumps[c].substr(fldWidth[c]), c1 = clumps.length; c1 > c + 1; --c1) clumps[c1] = clumps[c1 - 1];
                clumps[c + 1] = split
            }
        ...
        if (fill > 0)
            if (pad[c] == " ")
                for (var c1 = 0; c1 < fill; ++c1) clumps[c] += " ";
            else
                for (var c1 = 0; c1 < fill; ++c1) clumps[c] = pad[c] + clumps[c];
        external += clumps[c]
    }
    var clumpCount = clumps.length;
    while (clumpCount < expectedClumps) {
        for (var p = pad[clumpCount] == 0 ? " " : pad[clumpCount], c1 = 0; c1 < fldWidth[clumpCount]; ++c1) clumps[c] += p;
        ++clumpCount
    }
    for (var e = 0, formatted = new String(""), maskChars = new String("~`!@#$%^&()-_=+[]{}\\|;:'\"<>,./? "), m = 0; m < mask.length; ++m)
        if (maskChars.indexOf(mask.charAt(m)) >= 0) formatted += mask.charAt(m);
        else {
...
function GetCentury(year, additionalYears) {
    ...
    if (year < adjYear) return new String("20");
    else return new String("19")
}
...
function ModifyTimeClearFocus(obj) {
    ...
    if (objId) {
        ...
        if (mee && obj.masktype == AjaxControlToolkit.MaskedEditType.Time) {
            objValue = obj.value;
            if (objValue.indexOf("_") != -1) mee.set_ClearMaskOnLostFocus(false);
            else mee.set_ClearMaskOnLostFocus(true)
        }
    }
}
...
function ConfirmEntityDelete(entityName, index, deleteFunction, preDeleteValidationMethod) {
    if (preDeleteValidationMethod) {
        ...
        if (eval(funcName) == false) return false
    }
    var msg = "Are you sure you want to delete " + entityName + " #" + (index + 1) + "?";
    if (UbiTrialVehicle(index) == true) msg = "If you Remove this vehicle, we cannot apply the Snapshot discount earned.\nIf the customer has replaced this vehicle with another, then click Cancel and edit the vehicle information.";
    if (confirm(msg) == true) {
        ClearLastFocusField();
        var functionName = deleteFunction.substring(0, deleteFunction.indexOf("("));
        if (eval("typeof(" + functionName + ")") == "function") eval(deleteFunction);
        if (isVehiclePage(g_currentPage) && g_vehCount > 1) --g_vehCount;
        if (isDriverPage(g_currentPage) && g_drvrCount > 1) --g_drvrCount
    }
}
...
function OpenDialogOnFocus(callFunction) {
    ...
    if (eval("typeof(" + functionName + ")") == "function") eval("bShowPopup = " + callFunction);
    return bShowPopup
}

function TextDialogKeyPress(obj, dialogID) {
    if (eval("typeof(TextDialogKeyPress" + g_currentPage + ")") == "function") eval("TextDialogKeyPress" + g_currentPage + "('" + obj.id + "','" + dialogID + "')");
    else {
        if (event.keyCode == 9 || event.keyCode == 16 || event.keyCode == 17 || event.keyCode == 18 || event.keyCode == 27) return;
        PreventDefault();
...
function UpdateCheckBoxChanged(obj) {
    ...
    if (hiddenField)
        if (obj.checked) hiddenField.value = "Y";
        else if (obj.attributes["SetBlank"] && obj.attributes["SetBlank"].value == "true") hiddenField.value = "";
    else hiddenField.value = "N"
}
...
function SetFieldClear(FieldObject) {
    if (FieldObject.tagName == "INPUT") {
        if (FieldObject.type == "checkbox") {
            ...
            var emptyValue = "";
            if (FieldObject.getAttribute("mask") != null) emptyValue = GetEmptyMask(FieldObject);
...
function HandleRadioKeyDown() {
    if (EventObj().srcElement.keyCode == 13) return false
}
...
function IsDialogActive() {
    if (g_dialogIdList.length > 0) return true;
    return false
}

function GetActiveDialog() {
    var dialogId = "";
    if (IsDialogActive()) dialogId = g_dialogIdList[g_dialogIdList.length - 1];
    return dialogId
}
...
function OnMouseOverDialogPanel() {
    if (getIEVersionNumber() == 6) HideSelectListControls();
    else {
        var srcObj = GetObj("selectProtect");
        if (srcObj) srcObj.style.display = "none"
    }
}
...
function SetHiddenRenderDialogVariable(hiddenfieldValue) {
    ...
    if (GetObj(renderDialogId) != null) GetObj(renderDialogId).value = hiddenfieldValue
}
...
function ShowDialogWithEntity(srcElement, dialogId, forceShow, entity, mode) {
    ...
    g_dialogButton = "";
    if (srcElement) g_dialogButton = srcElement.id;
...
function OnFocusShowDialog(srcElement, dialogId, fieldId) {
    ...
    if (eval("typeof(" + functionName + ")") == "function") buttonAutoFireConditionList = eval(functionName + "()");
...
function HasDialogBeenDisplayed(hdnFieldId, dialogId, entity) {
    ...
    if (displayedDialogArray[entityIndex] == "Y") hasBeenDisplayed = true;
...
function CloseDialog(dialogId) {
    ...
    if (GetObj(dialogModeId) != null) GetObj(dialogModeId).value = "";
    ...
    if (g_dialogButton != "") g_lastFocusFieldId = g_dialogButton;
    g_dialogButton = "";
...
function ForceCloseDialog() {
    if (GetActiveDialog() == "PreBindingVerification") CallIfFunctionExists("PreBindingVerificationForceCloseDialog");
    else GetObj(GetHiddenVariable("OkButton" + GetActiveDialog())).click()
}
...
function IfFunctionExists(functionName) {
    if (eval("typeof(" + functionName + ")") == "function") return true;
    else return false
}
...
function CloseIfSysErrMsgDisplayed() {
    ...
    if (isSysErrMsgDisplayed != null) isSysErrMsgDisplayed.value == "Y" && self.close()
}

function SetCrossSell() {
    for (var parentNodeVar, mainMenuDiv = $get("mainMenuDiv"), menus = mainMenuDiv.getElementsByTagName("A"), menusCount = menus.length, index = 0; index < menusCount; index++) var menu = menus[index];
    menu.className = menu.className + " itemstylecrosssell";
    parentNodeVar = menu.parentNode;
    while (parentNodeVar.tagName.toLowerCase() != "table") parentNodeVar = parentNodeVar.parentNode;
...
function IsButton(obj) {
    if (obj && (obj.type == "image" || obj.type == "button" || obj.type == "submit")) return true;
    else return false
}

function IsAnchor(obj) {
    if (obj && obj.tagName == "A") return true;
    else return false
}

function IgnoreSpecialControlKeys(event) {
    if (event.ctrlKey)
        for (var i = 0; i < g_keysWithCtrl.length; i++) g_keysWithCtrl[i].toLowerCase() == String.fromCharCode(event.keyCode).toLowerCase() && StopPropagationAndPreventDefault()
}
...
function IdentifyWindowCloseKeys() {
    try {
        ...
        if (bAltKey && keyCode == g_keyF4 || bCtrlKey && keyCode == g_keyW || bCtrlKey && keyCode == g_keyw || bCtrlKey && keyCode == g_keyF4) g_isWindowCloseKey = true;
        else g_isWindowCloseKey = false
    } catch (ex) {}
}
...
function IdentifyWindowUnloadKeys() {
    switch (window.event.keyCode) {
        ...
        case g_keyRightArrow:
            if (window.event.altKey) g_isWindowUnloadKey = true;
            break;
        default:
            g_isWindowUnloadKey = false
    }
...
function SetUnloadkeyCloseStatus() {
    if (g_isWindowUnloadKey == false) g_isWindowUnloadKey = true
}
...
function IsWindowCloseClicked() {
    ...
    if (!g_isWindowUnloadKey && !g_insideWindowUnload) returnValue = true;
    if (g_isWindowCloseKey == true) returnValue = true;
...
function wheel() {
    if (IsExpandListOpened()) {
        if (!HitTest()) window.event.returnValue = false
...
function ResizeMainDivHeight() {
    var bodyHeight = document.documentElement.clientHeight,
        bannerHeight = 0,
        tabListHeight = 0;
    if (GetObj(g_bannerId)) bannerHeight = GetObj(g_bannerId).clientHeight;
    if (GetObj(g_menuDivId)) tabListHeight = GetObj(g_menuDivId).clientHeight;
    var newMainDivWrapperHeight = bodyHeight - (bannerHeight + tabListHeight + GetHeightOffset());
    if (newMainDivWrapperHeight > 0) GetObj("mainDivWrapper").style.height = newMainDivWrapperHeight + "px";
    else GetObj("mainDivWrapper").style.height = "auto"
}
...
function ResizeMainDivWidth(offset) {
    if (offset == null) offset = GetPageOffset();
    ...
    if (newWidth != mainDivWidth) mainDiv.style.width = newWidth + "px"
}
...
function openSpawnWindow(_page, w, h) {
    ...
    var winparms = "";
    if (inHouse)
        if (inHouse.value == "Y") winparms = "status=yes, resizable=yes, scrollbars=yes, width=" + w + ", height=" + h + ",left=" + 0 + ",top=" + 32;
        else winparms = "status=yes, resizable=yes, scrollbars=yes, width=" + w + ", height=" + h + ",left=" + 0 + ",top=" + 0;
    else winparms = "status=yes, resizable=yes, scrollbars=yes, width=" + w + ", height=" + h + ",left=" + 0 + ",top=" + 0;
...
function ShowPopupCenterWindow(obj, url, windowName, windowWidth, windowHeight, windowOptions, needApplicationContext) {
    if (needApplicationContext == null || needApplicationContext == "undefined" || needApplicationContext == true) url = AddApplicationContextIndex(url);
...
function ShowModalPopupWindow(url, windowName, windowWidth, windowHeight, parentObjectParam) {
    ...
    } else parentObject = parentObjectParam;
    try {
        var retval = "",
            prntNewWindow = false;
        typeof PrintWindowNeeded == "function" && PrintWindowNeeded() == true ? (prntNewWindow = true) : (prntNewWindow = false);
        if (window.showModalDialog && !prntNewWindow && GetObjValue("mobiledevice_ind") != "Y") retval = window.showModalDialog(url, parentObject, windowProperties);
...
function SetLoadingBody(obj, url) {
    if (obj) {
        ...
        if (url.indexOf("create_forms.htm") != -1) body = "Please Wait<br> Creating Document(s).<br> <img src='images/plswait.gif'>";
        obj.innerHTML = body
    }
}
...
function SetLoadingTitle(obj, url) {
    if (obj) {
        ...
        if (url.indexOf("trxInFlight.htm") != -1) title = "Transaction in Process";
...
function AddApplicationContextIndex(currentUrl) {
    ...
    if (applicationIndex && currentUrl.indexOf("applicationContextIndex") < 0)
        if (currentUrl.indexOf("?") >= 0) currentUrl = currentUrl + "&applicationContextIndex=" + applicationIndex;
        else currentUrl = currentUrl + "?applicationContextIndex=" + applicationIndex;
    return currentUrl
}
...
function BrowserSupprtHighlightOnFocus() {
    if (_highlightOnFocus == null) {
        ...
        if (navigator.userAgent.match(/msie/i)) _highlightOnFocus = false;
        if (_highlightOnFocus != true && navigator.userAgent.toString().toLowerCase().indexOf("trident/6") > -1) _highlightOnFocus = true
    }
...
function FieldOnFocus(obj) {
    ...
    if (!BrowserSupprtHighlightOnFocus() && obj.tagName != "SELECT") field.className = field.className + " outline";
    SetPageFocusField(obj);
    if (obj.type != "select") g_currentlyFocusedList = "";
    try {
        if (getIEVersionNumber() == 6) {
            var selectProtectIframe = GetObj("expandList~iframe");
            if (selectProtectIframe.contentWindow.GetSrcSelect() == obj.id) selectProtectIframe.contentWindow.document.all("selectValue").className = "all clsSelectProtectWithFocus"
        }
    } catch (ex) {}
}
...
function StoreLastFocusField(obj) {
    if (obj) g_lastFocusFieldId = obj.id
}
...
function IsEnabled(obj) {
    if (obj) return !obj.disabled;
    return false
}

function IsVisible(obj) {
    if (obj && obj.style.display != "none" && obj.style.visibility != "hidden" && (obj.type == null || obj.type != "hidden")) return true;
    return false
}
...
function AreAncestorsVisible(obj) {
    ...
    if (parentObj != null && parentObj.tagName != "BODY" && parentObj.tagName != "FORM")
        if (IsVisible(parentObj)) return AreAncestorsVisible(parentObj);
        else return false;
    return true
}

function IsFocusable(obj) {
    if (obj && IsFocusableControlType(obj) && IsEnabled(obj) && IsVisible(obj) && AreAncestorsVisible(obj)) return true;
    return false
}
...
function IsHighlightedControl(obj) {
    if (obj && obj.tagName == "INPUT" && obj.type == "text") return true;
    return false
}
...
function FocusControl(targetControlId) {
    var targetControl = GetObj(targetControlId),
        focused = false;
    if (targetControl && IsFocusable(targetControl)) try {
        if (Sys.Browser.agent === Sys.Browser.InternetExplorer) {
            ...
            targetControl.focus();
            if (focusTarget) focusTarget.contentEditable = oldContentEditableSetting
...
function FocusChildControl(targetControlId, rootObj) {
    var targetControl = GetObj(targetControlId),
        focused = false;
    if (rootObj == null) rootObj = document.documentElement;
    if (targetControl)
        if (AjaxControlToolkit.DomUtility.isAncestorOrSelf(targetControl, rootObj)) focused = FocusControl(targetControlId);
    return focused
}
...
function FocusFirstControl(rootObj) {
    for (var form = document.forms[0], i = 0; i < form.elements.length; i++) {
        ...
        if ($(thisObj).parents != "undefined" && $(thisObj).parents("table[handletabs]").length > 0 && $(thisObj).closest("td").index() > 1) continue;
        if (FocusChildControl(thisObj.id, rootObj)) break
    }
}
...
function AdjustWrapper(scrollWidth, scrollHeight, wrapperDiv, innerDiv, scrollBarPadding, divWrapperAdj, preset, alwaysPadForScroll) {
    ...
    if (divWrapperAdj == null) divWrapperAdj = 0;
    ...
    if (height > scrollHeight && scrollHeight != 0) {
        height = scrollHeight;
        width += scrollBarSize;
        if (width > scrollWidth && scrollWidth != 0) width = scrollWidth
    }
    wrapperDiv.style.width = width + "px";
    wrapperDiv.style.height = height + "px"
}
...
function ClearInputData(rootObj) {
    for (var children = rootObj.childNodes, i = 0; i < children.length; i++)
        if (children[i].tagName == "INPUT") {
            if (children[i].attributes["Clear"] && children[i].attributes["Clear"].value == "true")
                ...
                var emptyValue = "";
                if (children[i].getAttribute("mask") != null) emptyValue = GetEmptyMask(children[i]);
    ...
    } else children[i].childNodes.length > 0 && ClearInputData(children[i])
}
...
function DisableInputControls() {
    ...
    for (i = 0; i < document.forms[0].elements.length; i++) {
        ...
        if (IsFocusable(obj)) !obj.disabled && Sys.UI.DomElement.addCssClass(obj, "disabled")
    }
}
...
function EnableInputControls() {
    ...
    for (i = 0; i < document.forms[0].elements.length; i++) {
        obj = document.forms[0].elements[i];
        if (IsFocusable(obj)) !obj.disabled && Sys.UI.DomElement.removeCssClass(obj, "disabled")
    }
}
...
function SetInsideWindowUnload(switchFlag) {
    if (switchFlag) g_insideWindowUnload = true;
    else g_insideWindowUnload = false
}

function SetPopupBlockerInd(switchFlag) {
    if (switchFlag) g_popupBlockerUnload = true;
    else g_popupBlockerUnload = false
}

function SetCloseWindowIndicator(switchFlag) {
    if (switchFlag) g_closeWindow = true;
    else g_closeWindow = false
}
...
function SetAdobeInstallCheckField() {
    ...
    if (adobeInstalled.value != "") return;
    if (IsAcrobatReaderInstalled()) adobeInstalled.value = "Y";
    else adobeInstalled.value = "N"
}
var g_disablePageId;
...
function DisablePage() {
    ...
    if (g_disablePageMutex == 0) behavior != null && behavior.show();
    g_disablePageMutex++
}

function EnablePage() {
    g_disablePageMutex--;
    if (g_disablePageMutex < 0) g_disablePageMutex = 0;
...
function SetPageFocusField(field) {
    ...
    if (_obj)
        if (field.getAttribute("name")) _obj.value = field.getAttribute("name");
        else _obj.value = field.getAttribute("id")
}
...
function InetCookie() {
    ...
    function _setCrumb(key, val) {
        ...
        if (myDomain.length > 0) document.cookie = key + "=" + escape(val) + ";path=/;domain=" + myDomain;
        else document.cookie = key + "=" + escape(val) + ";path=/"
    }
...
function deleteCookie(name) {
    if (getCookie(name)) {
        var myDomain = BaseDomainString();
        if (myDomain.length > 0) document.cookie = name + "=;expires=Thu, 01-Jan-1970 00:00:01 GMT;path=/;domain=" + myDomain;
        else document.cookie = name + "=;expires=Thu, 01-Jan-1970 00:00:01 GMT;path=/"
    }
}
...
function setCookie(name, value, expires, path, domain, secure) {
    ...
    if (expires) expires = expires * 1e3 * 60 * 60 * 24;
...
var g_keyPageUp = 33,
    ...
    g_keyW = 87,
    g_keyw = 119,
    g_keyCode, g_bAltKey, g_bEscapeKey, g_bTabKey, g_keyShift = 16,
    g_keyCntrl = 17,
    g_keyAlt = 18,
    g_keyEscape = 27,
    g_scrollbarWidth = 16,
    g_statusBarHeight = 24,
    g_mainDivWrapperHorizontalScrollBarThreshold = 8,
    g_mainDivWrapperVerticalScrollBarThreshold = 124,
...
function setDefaultSelectValue(objSrcList, updateDependancies, defaultValueWhenEmpty) {
    if (objSrcList.selectedIndex != -1 && objSrcList.selectedIndex != 0) return;
    ...
    if (iOptions == 0) return;
    if (iOptions == 1) objSrcList.selectedIndex = 0;
    if (iOptions == 2 && objSrcList.options[0].text == "") objSrcList.selectedIndex = 1;
    if (bSetDefault)
        if (defaultValueWhenEmpty == undefined || defaultValueWhenEmpty == "") SelectFirstNonBlankValue(objSrcList);
        else SetSelectListEmptyDefaultValue(objSrcList, defaultValueWhenEmpty);
    if (originalSelIndex != objSrcList.selectedIndex) {
        FldOnChange(objSrcList, updateDependancies);
        if (bSetDefault && objSrcList.OnDefault) eval(objSrcList.OnDefault)
    }
}

function SelectFirstNonBlankValue(objSrcList) {
    if (objSrcList.options[0].text == "") objSrcList.selectedIndex = 1;
    else objSrcList.selectedIndex = 0
}
...
function SetSelectListEmptyDefaultValue(objSrcList, defaultValueWhenEmpty) {
    ...
    if (dfltIdx == -1) SelectFirstNonBlankValue(objSrcList);
    else objSrcList.selectedIndex = dfltIdx
}

function SelectFirstItem(objExpandList) {
    if (objExpandList.selectedIndex == -1)
        if (objExpandList.options.length > 0) objExpandList.selectedIndex = 0
}
...
function showExpand(objSrcList) {
    if (objSrcList.AutoWidth == "N" || g_pausePositionSelectProtect == true) return;
    g_bEscapeKey = false;
    g_bTabKey = false;
...
function ShowExpandList(objSrcList) {
    ...
    if (objSrcList.selectedIndex != -1) commonSelect.selectedIndex = objSrcList.selectedIndex;
    else if (commonSelect.options.length > 0 && objSrcList.SetDefaultValue == "true") commonSelect.selectedIndex = 0;
...
function PositionExpandList(objSrcList, commonContainer, commonSelect) {
    ...
    if (getIEVersionNumber() != 6) commonContainer.style.clip = "rect(0," + commonWidth + "," + commonHeight + ",0)";
    ...
    if (widthNeeded > document.documentElement.clientWidth)
        if (srcAbsLeft + srcWidth - commonWidth - verticalScrollbarWidth > 0) commonLeft = srcAbsLeft + srcWidth - commonWidth;
        else {
            commonLeft = 0;
    ...
    if (heightNeeded > document.documentElement.clientHeight) commonTop = srcAbsTop - commonHeight;
...
function SetSelectListHeaderContent(commonContainer, objSrcList, commonWidth) {
    ...
    arr[1] = 0;
    try {
        var selHeaderTbl = null,
            srcHdrTbl = GetObj(objSrcList.id + "_tblSW"),
            commenSelect;
        if (getIEVersionNumber() != 6) {
        ...
        if (srcHdrTbl == null) {
            ...
            if (commenSelect.className.indexOf("expListFixed", 0) != -1) commenSelect.className = commenSelect.className.replace(/expListFixed /, "");
            arr[0] = 0;
            arr[1] = 0;
            return arr
        }
        ...
        for (var iLen = 0, i = 0; i < commenSelect.options.length; i++) {
            ...
            if (iLen < txt.length) iLen = txt.length
        }
...
function closeExpandList(commonSelect, bSetSelected) {
    ...
    if (getIEVersionNumber() != 6) GetExpandListDiv().style.display = "none";
    ...
    if (bSetSelected)
        if (commonSelect.selectedIndex != -1 && (originalSelect.AllowBlankOption == "true" || commonSelect.value != "")) {
            ...
            if (originalSelect.selectedIndex != commonSelect.selectedIndex) selChanged = true;
            originalSelect.selectedIndex = commonSelect.selectedIndex;
            if (selChanged == true) {
                selectedText = commonSelect.options[commonSelect.selectedIndex].text;
                if (document.createEventObject) originalSelect.fireEvent("onchange");
                else {
...
function showOriginalSelect(commonSelect, bSetSelected) {
    ...
    } else SetFocusToObj(originalSelect)
}
...
function PlaceSelectProtect(objSrcList) {
    ...
    if (getIEVersionNumber() == 6) {
        var iframe = GetObj("expandList~iframe"),
            selectedText = "";
        if (objSrcList.selectedIndex != -1) selectedText = objSrcList.options[objSrcList.selectedIndex].text;
        try {
        ...
        } else iframe.style.clip = "rect(0," + objSrcList.offsetWidth + "," + objSrcList.offsetHeight + ",0)";
        try {
            if (g_currentlyFocusedList == objSrcList.id || document.activeElement.id == objSrcList.id) iframe.contentWindow.document.all("selectValue").className = "all clsSelectProtectWithFocus";
            else iframe.contentWindow.document.all("selectValue").className = "all"
        } catch (ex) {}
...
function SimulateSelectClick(hiddenSelect) {
    if (hiddenSelect != "") {
        if (g_pausePositionSelectProtect == true) return false;
...
function GetHorizontalScrollbarHeightIfVisible() {
    ...
    if (mainDivWrapper != null && mainDiv != null) {
        ...
        if (mainDivWrapperBounds.width - mainDivBounds.width < g_mainDivWrapperHorizontalScrollBarThreshold) horizontalScrollbarHeight = g_scrollbarWidth
    } else horizontalScrollbarHeight = g_scrollbarWidth;
...
function GetVerticalScrollbarWidthIfVisible() {
    ...
    if (mainDivWrapper != null && mainDiv != null) {
        var mainDivWrapperBounds = GetContentBounds(mainDivWrapper),
            mainDivBounds = GetContentBounds(mainDiv);
        if (mainDivWrapperBounds.height - mainDivBounds.height < g_mainDivWrapperVerticalScrollBarThreshold) verticalScrollbarWidth = g_scrollbarWidth
    } else verticalScrollbarWidth = g_scrollbarWidth;
...
function ScrollToMakeActiveSelectVisible(objSrcList) {
    ...
    ptPosArr = $common.getLocation(objSrcList);
    srcAbsLeft = ptPosArr.x;
    if (srcAbsLeft < 0) mainDivWrapper.scrollLeft = mainDivWrapper.scrollLeft + srcAbsLeft - 1;
    ...
    if (srcAbsTop < mainDivWrapperBounds.y) mainDivWrapper.scrollTop = topScrollPos - (mainDivWrapperBounds.y - srcAbsTop) - 1
}
...
function IFrameSimulateSelectClickPart2(SrcList) {
    ...
    if (objSrcList.selectedIndex != -1) GetIframeExpandList().selectedIndex = objSrcList.selectedIndex;
    else if (GetIframeExpandList().options.length > 0 && objSrcList.SetDefaultValue == "true") GetIframeExpandList().selectedIndex = 0;
    ...
    } else closeHandlerIframe.style.clip = "rect(0," + objSrcList.offsetWidth + "," + objSrcList.offsetHeight + ",0)";
...
function EnableNativeSelect(nativeSelectId) {
    ...
    if (getIEVersionNumber() != 6) SimulateSelectClick(nativeSelectId);
    else IFrameSimulateSelectClick(nativeSelectId)
}
...
function HideExpandListAndShrinkElement(elementId) {
    ...
    if (getIEVersionNumber() == 6) {
        ...
        if (objExpandList != null) objExpandList.style.display = "none"
    } else GetExpandListDiv().style.display = "none";
...
function HandleKeyDown(srcSelect, strHandleSelectListKeyDown) {
    ...
    if (eval("typeof(" + strHandleSelectListKeyDown + ")") == "function") eval(strHandleSelectListKeyDown + "(srcSelect)")
}
...
function HandleSrcSelectKeyDown(srcSelect) {
    ...
    switch (g_keyCode) {
        case g_keyDownArrow:
            if (getIEVersionNumber() == 6 && IsDialogActive()) return;
        ...
        case g_keyTab:
            g_currentlyFocusedList = "";
            if (getIEVersionNumber() == 6) window.setTimeout("HideSelectListControls()", 5);
            else GetExpandListDiv().isOpen == "Y" && window.setTimeout("HideSelectListControls()", 5);
            break;
        default:
            getIEVersionNumber() == 6 && window.setTimeout(commandString, 5)
    }
...
function handleTabKey(commonSelect) {
    showOriginalSelect(commonSelect, true);
    var newEvent;
    if (document.createEventObject) newEvent = document.createEventObject(event);
    else newEvent = document.createEvent(event);
...
function SetExpandListHeight(expList) {
    expList.size = 10;
    if (expList.options.length < expList.size) expList.size = expList.options.length;
    if (expList.options.length <= 2) expList.size = 2
}

function clearCommonSelect(commonSelect) {
    while (commonSelect.options.length > 0) commonSelect.remove(0)
}
...
function DisableGroupHeadings(objSrcList) {
    var opt = null;
    if (objSrcList.options != null)
        for (var i = 0; i < objSrcList.options.length; i++) objSrcList.options[i].disabled = PopulateAttributes(objSrcList.options[i].text)
}
...
if (document.implementation.hasFeature("XPath", "3.0")) {
    XMLDocument.prototype.selectNodes = function(cXPathString, xNode) {
        if (!xNode) xNode = this;
        for (var oNSResolver = this.createNSResolver(this.documentElement), aItems = this.evaluate(cXPathString, xNode, oNSResolver, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null), aResult = [], i = 0; i < aItems.snapshotLength; i++) aResult[i] = aItems.snapshotItem(i);
        return aResult
    };
...
function GetElementByIdOrName(id, formName) {
    var obj = GetObj(id);
    if (!obj)
        if (formName) return document.forms[formName].elements[id];
        else return document.forms[0].elements[id];
    else return obj
}
...
function IsValidAllNode(node) {
    if (node.nodeType == 3 && node.nodeValue.trim().length == 0) return false;
    return true
}

function GetDocumentAllObjects(idStr) {
    var objects;
    if (document.all) objects = document.all(idStr);
    if (objects == null || objects.length == null) objects = document.getElementsByName(idStr);
    return objects
}

function AddEvent(obj, event, callback) {
    if (obj.addEventListener) obj.addEventListener(event, callback, true);
    else obj.attachEvent(event, callback)
}
...
function DetachEvent(obj, event, callback) {
    if (obj.removeEventListener) obj.removeEventListener(event, callback, true);
    else obj.detachEvent(event, callback)
}
...
function EventClass() {
    function SetCustomKey(val) {
        if (val != null)
            ...
            } catch (e) {} else GlobalEventHandler.event.keyCode = val;
        if (!window.event) return GlobalEventHandler.event.which;
        else return GlobalEventHandler.event.keyCode
    }
    if (!window.event) GlobalEventHandler.event.srcElement = GlobalEventHandler.event.target;
...
function FilterDataIsland(changedObj, dataIsland, xPath) {
    var xml = GetPageFieldRelationships(),
        ...
        masterNodes = null;
    if (coverageExtractFilterDocument != null) masterNodes = coverageExtractFilterDocument.selectNodes(xPath);
    var changedFieldId = GetBaseFieldId(changedObj),
    ...
    if (masterNodes != null && masterNodes.length > 0) {
        ...
        if (coverageExtractFilterDocument != null) masterNodes = coverageExtractFilterDocument.selectNodes(masterXpath);
        if (masterNodes != null && masterNodes.length > 0)
...
function IsSiblingRequired(changedObj) {
    for (var parts = changedObj.name.split("."), lastPos = GetGroupCount(changedObj), sourceControl = changedObj, j = 1; j < lastPos; j++) {
        ...
        if (destControl.disabled == false) return false
    }
    return true
}
...
function GetClientControl(fieldId) {
    ...
    if (IsDialogActive()) newFieldId = GetActiveDialog() + g_prefixSeparator + fieldId;
    for (var prefixes = GetControlPrefixes(), i = 0; i < prefixes.length; i++) {
...
function AppendDialogStyles(strCtrlPrefix, strCtrlId, strCtrlStyle) {
    ...
    if (dlgCtrl) dlgCtrl.className = dlgCtrl.className + strCtrlStyle
}
...
function IsRadioButton(object) {
    if (document.getElementsByName(object.id + "_radio").length > 0) return true;
    else return false
}

function IsCheckBox(object) {
    if (document.getElementsByName(object.id + "_displayCheckBox").length > 0) return true;
    else return false
}
...
function setRadioValue(theObj, theObjValue) {
    for (var i = 0; i < theObj.length; i++)
        if (theObj[i].value == theObjValue) theObj[i].checked = true;
        else theObj[i].checked = false
}

function DisableRadioCollection(theObj) {
    for (var i = 0; i < theObj.length; i++) theObj[i].disabled = true
}

function EnableRadioCollection(theObj) {
    for (var i = 0; i < theObj.length; i++) theObj[i].disabled = false
}
...
function ShowElement(element, needEnable) {
    if (element) {
        ...
        if (needEnable) {
            var controls, controlsCount = 0,
                elementToEnable = element;
...
function GetAttribute(objHTMLElement, strAttribute) {
    var attribute = "";
    if (objHTMLElement.getAttribute(strAttribute) != null) attribute = objHTMLElement.getAttribute(strAttribute);
    return attribute
}
...
function CancelFaoMenuEventBubble() {
    ...
    if (faoMenuIndicator)
        if (window.event != null) {
            if (window.event.cancelBubble !== undefined) window.event.cancelBubble = true;
    ...
    if (Sys.Browser.agent == Sys.Browser.Webkit && (scanCode == 8 || scanCode == 46 || scanCode == 127)) {
        ...
        if (maskedEdit != undefined) {
            ...
            if (scanCode == BACKSPACE_KEY) {
                ...
                if (curpos != -1) ele.setSelectionRange(curpos, curpos);
                else {
                ...
                if (curpos == -1) {
                    ...
                    if (!maskedEdit._isValidMaskedEditPosition(curpos))
                        if (curpos != maskedEdit._LogicLastInt && maskedEdit._InputDirection != AjaxControlToolkit.MaskedEditInputDirections.RightToLeft) curpos = maskedEdit._getNextPosition(curpos);
                    ...
                    if (ClearText != "") {
                        ...
                        } else maskedEdit.loadValue(ClearText, maskedEdit._LogicLastPos)
                    }
                }
                ele.setSelectionRange(curpos, curpos);
    ...
    if (Sys.Browser.agent == Sys.Browser.Webkit) {
        ...
        if (maskedEdit != undefined)
            if (maskedEdit._InputDirection == AjaxControlToolkit.MaskedEditInputDirections.RightToLeft)
                if (maskedEdit._LogicLastInt != -1) setSelectionRange(maskedEdit._LogicLastInt, tmaskedEdithis._LogicLastInt);
                else setSelectionRange(maskedEdit._LogicLastPos + 1, maskedEdit._LogicLastPos + 1);
        ...
        } else setSelectionRange(maskedEdit._LogicFirstPos, maskedEdit._LogicFirstPos)
    }
}
...
function FormatInput(spec, mask, value) {
    if (value != "") return Format(spec, mask, value);
    return value
}
...
function FormatInputTime(spec, mask, value) {
    ...
    var ampm = " AM";
    if (value.toLowerCase().indexOf("p") > -1) ampm = " PM";
...
function DisallowBlankOption(obj) {
    if (obj.selectedIndex == 0 && obj.value == "" && obj.getAttribute("AllowBlankOption") == "false") {
        ...
        if (GetObjValue("mobiledevice_ind") === "Y") obj.value = obj.getAttribute("prevVal");
        ...
        } else if (evt.keyCode == g_keyDownArrow || evt.keyCode == g_keyUpArrow || evt.type == "change") obj.value = obj.getAttribute("prevVal")
    } else obj.setAttribute("prevVal", obj.value)
}
                        ...
                        $(this).off("keydown").on("keydown", function(e) {
                            ...
                            if (code == 9) {
                                var rId = rIdTmp,
                                    ...
                                    iRws = iRwsTmp,
                                    iCls = iClsTmp;
                                if (!e.shiftKey) FindNextActiveInput(e, rId, cId, iRws, iCls);
                                else FindPrevActiveInput(e, rId, cId, iRws, iCls)
                            }
                        })
                    })
                })
...
function FindNextActiveInput(evt, r, c, rows, cols) {
    try {
        ...
        if (r < rows - 1) {
            ...
            if (tgtSrcFromNextCol.length === 0 || tgtSrcFromNextCol.is(":disabled")) FindNextActiveInput(evt, r + 1, c, rows, cols);
            else {
...
function FindPrevActiveInput(evt, r, c, rows, cols) {
    try {
        ...
        if (r > 0) {
            ...
            if (tgtSrcFromPrevCol.length === 0 || tgtSrcFromPrevCol.is(":disabled")) FindPrevActiveInput(evt, r - 1, c, rows, cols);
            else {
                var tgtSrcFromCurrentCol = $($(evt.target).closest("table")[0].rows[r].cells[c]).find("input:visible, select:visible");
                if (HandlePrevForMultiInputs(evt, tgtSrcFromCurrentCol, false) || HandlePrevForMultiInputs(evt, tgtSrcFromPrevCol, true)) return false;
...
function isPdfMimeTypeSupported() {
    if (navigator.mimeTypes != null && navigator.mimeTypes.length > 0)
        for (i = 0; i < navigator.mimeTypes.length; i++) {
            ...
            if (mtype.type == "application/pdf" && mtype.enabledPlugin) return true
        }
    if (navigator.appVersion.indexOf("Edge/") > -1) return true;
    return false
}
...
function EntityCancel() {
    if (g_vehCount > 1 && g_currentPage == "C2VehicleSummary") --g_vehCount;
    if (g_drvrCount > 1 && g_currentPage == "C2DriverSummary") --g_drvrCount
}
...
function PostScreenNotEval() {
    if (eval("typeof(PostScreen)") == "function") return eval(PostScreen(navigateMenuId));
    return true
}
...
var Action, homeWin, waitID, rtnToCaller = false,
    KeepAQAROpen = false,
    transactionComplete = false,
    PHAAgentQuoteHasTeaserRateWithHomeOwner;
...
function HomeownerClick() {
    ...
    if (ShouldStartNewPhaQuote()) ShowDialog(null, "DwellingInfo", true);
    else RedirectToHomeownerPortal("QuoteSummary")
}
...
function RedirectToHomeownerPortal(act, rtrn) {
    DisableAll();
    if (rtrn != "undefined") rtnToCaller = rtrn;
    else rtnToCaller = false;
    if (OkToCallAuthenticate()) try {
        Action = act;
...
function ShouldKeepAqarOpen() {
    if (IsPlatinumQuote()) return false;
    else return true
}
...
function ProcessSignon(result) {
    ...
    if (result == "SessionExpired") window.location = "SessionExpired.htm";
        ...
        try {
            ...
            if (rsp != null) {
                ...
                rsp.preserveWhiteSpace = true;
                rsp.loadXML(result)
            } else rsp = (new DOMParser).parseFromString(result, "text/xml")
        } catch (exc) {
            rsp = (new DOMParser).parseFromString(result, "text/xml")
        }
        var homeUrl, hmToken, hmAgtCd, hmQtNm, hmPolNm, hmPolNmSfx, authErr = "Homeowner Portal Authentication Failure",
            partnerErr = "PHA quote is no longer valid because you now write with a new PHA underwriting company.  Please cross sell this customer for a new PHA quote.";
        if (rsp) {
    ...
    try {
        ...
        if (homeWin == null) {
            EnableAll();
            if (hpQuoteMsg) hpQuoteMsg.innerText = "You may be receiving this message due to a Pop-up blocker.  To avoid this in the future, please add ForAgentsOnly.com to your exclusion list.";
...
function FocusOnHomeownerPortal() {
    try {
        if (homeWin && !homeWin.closed) homeWin.focus();
        else DetachDOMEvent("onfocus", FocusOnHomeownerPortal)
    } catch (e) {
...
function ReturnFromHomeownerPortal() {
    try {
        if (homeWin.closed) {
            ...
            if (ShouldRunUpdatePolicyInfoTransaction()) RunUpdatePolicyInfoTransaction(true);
            else rtnToCaller && ReturnToCaller()
        }
    } catch (e) {
        EnableAll();
        var close = e
...
function RunUpdatePolicyInfoTransaction(useCallBack) {
    try {
        if (useCallBack) Progressive.Agent.Quoting.Web.Aqar.FieldInfoService.UpdatePolicyInfo(GetQueryStringArgs().applicationContextIndex, GetqueryStringValue("syncId"), ProcessUpdatePolicyInfoTransaction, HomeownerPortalError);
        else {
...
function BeforeClosingAQAR() {
    if (KeepAQAROpen && homeWin && !homeWin.closed)
        if (GetObjValue("calling_app") == "") event.returnValue = "The auto quote must be kept open while quoting Homeowners.";
        else event.returnValue = "ForAgentsOnly.com must be kept open while processing Homeowners."
}
...
function SetRedirectFailureInd(val) {
    var redirectFailureInd = GetObj("HomeownerPortal.0.hp_redirect_failure_ind");
    if (redirectFailureInd != null) redirectFailureInd.value = val
}

function SetShowPopupBlockerMsg(val) {
    var showPopupBlockerMsg = GetObj("HomeownerPortal.0.hp_show_popup_blocker_msg");
    if (showPopupBlockerMsg != null) showPopupBlockerMsg.value = val
}

function ShouldRunUpdatePolicyInfoTransaction() {
    if (GetObj("HomeownerPortal.0.hp_quote_id").value == "" || GetObj("HomeownerPortal.0.hp_partner_qt_stat").value == "Sold") return false;
    return true
}
...
function getMaxLength(_this, _maxLength) {
    if (!_maxLength) return _this.maxlen;
    return _maxLength
}

function getMaxRows(_this, _maxRows) {
    if (!_maxRows) return _this.maxRows;
    return _maxRows
}
...
function textareaSize(_this, _maxLength, _maxRows) {
    ...
    if (!_this.allowReturns && asciicode == g_keyEnter) return false;
    if (asciicode <= 31 && asciicode != g_keyEnter) return true;
    if (_maxRows != 0) {
        var wrkString = _this.value + String.fromCharCode(asciicode);
        if (ExceedMaxRows(_this, wrkString, _this.cols, _maxRows, false)) return false
    }
...
function onBlurTextarea(_this) {
    for (var newValue = "", i = 0; i < _this.value.length; i++) {
        ...
        if (!_this.allowReturns) {
            if (currChar == "\n") newValue += " ";
            else if (currChar != "\r") newValue += currChar
        } else newValue += currChar
    }

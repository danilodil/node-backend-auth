/* eslint-disable no-plusplus, no-mixed-operators, object-shorthand, no-shadow, curly, prefer-const, no-else-return, no-console, no-await-in-loop, no-loop-func, guard-for-in, max-len, no-use-before-define, no-undef, no-inner-declarations,no-nested-ternary,
 no-param-reassign, guard-for-in ,no-prototype-builtins, no-return-assign, prefer-destructuring, no-restricted-syntax, no-constant-condition, dot-notation, quotes, func-names, consistent-return, no-unneeded-ternary, nonblock-statement-body-position, no-floating-decimal, arrow-parens */
      ...
      async function collectStep() {
        try {
          ...
          // }
            ...
            if (errorEl !== undefined && errorEl) {
              console.log('EEL ', errorEl);
            } else {
              ...
              for (let j = 0; j < pageArray.length; j++) {
                const pageNo = (j + 1);
                // const pageNo = 1;
                const pageUrl = `https://barlist.com/independent-agents/search?stateid=${stateId}&countryid=1&page=${pageNo}`;

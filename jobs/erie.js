...
const erieQueue = new Queue('erie', ENVIRONMENT.redisUrl);
...
erieQueue.process(maxJobsPerWorker, async (job, done) => {
  try {
    await erie(job.data);
    done();
  } catch (e) {
...
async function erie(req) {
  try {
    ...
    function populateData() {
      ...
      dataObj.gender = { element: '#selGender1', value: bodyData.gender ? bodyData.gender.charAt(0) : staticDataObj.gender };

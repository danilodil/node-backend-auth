...
const safecoQueue = new Queue('safeco', ENVIRONMENT.redisUrl);
...
safecoQueue.process(maxJobsPerWorker, async (job, done) => {
  try {
    await safeco(job.data);
    done();
  } catch (e) {
...
async function safeco(req) {
  try {
    console.log('Added to safecoQueue');
